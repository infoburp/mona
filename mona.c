...
void x_init(void)
{
	...
	win = XCreateWindow(dpy, DefaultRootWindow(dpy), 0, 0, WIDTH, HEIGHT,
	                    ...
	                    DefaultVisual(dpy, screen), CWBackPixmap, &attr);
    ...
    // mutate shape
		...
		if(roulette < 1.5) {
			...
			if(drastic < 1) {
				...
				dna_test[mshape].ps[p_i].y =
				 CLAMP(dna_test[mshape].ps[p_i].y, 0, HEIGHT-1);
			} else {
				dna_test[mshape].ps[p_i].y = RANDDOUBLE(HEIGHT);
			}
		}
    // mutate stacking
	} else {
		int destination = RANDINT(NUM_SHAPES);
		shape_t s = dna_test[mshape];
